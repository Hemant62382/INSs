...
const CareerCoursePage = () => {
  ...
  heroSection: {
    ...
    description:
      "No IT background? No problem! BA, B Com., BSc graduates can become network engineer in 1 year!",
    highlights: [
  ...
  return (
    <div className="font-sans bg-gray-800 antialiased">
      {/* Hero Section */}
      <section className="bg-gradient-to-r from-indigo-900 to-indigo-700 text-white py-20 px-4 sm:px-6 lg:px-8">
        <div className="max-w-7xl mx-auto text-center">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6 mt-12">
            {courseData.heroSection.highlights.map((item, index) => (
        <div
          key={index}
          className="bg-white/10 p-6 rounded-xl backdrop-blur-sm border border-white hover:bg-white/20 transition-all duration-300"
        >
          <p className="font-semibold text-center text-lg">{item}</p>
        </div>
      ))}
          </div>
        </div>
          ...
          <div className="space-y-10 relative">
            ...
            {courseData.process.steps.map((step, index) => (
              <div key={index} className="relative pl-12 md:pl-24">
                ...
                <div className="bg-white/50 p-8 rounded-xl shadow-sm border border-l-4 border-amber-800 border-l-indigo-600 hover:shadow-2xl hover:bg-white transition-all">
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {courseData.jobReady.features.map((feature, index) => (
              <div 
                key={index} 
                className="bg-white p-8 rounded-xl shadow-sm hover:shadow-xl hover:scale-105 transition-all duration-300 border border-gray-100 hover:border-indigo-100"
              >
        ...
        <div className="max-w-4xl mx-auto text-center">
          ...
          <div className="flex flex-col sm:flex-row justify-center gap-4">
            <a
              href="/ACNP.pdf"
              download="ACNP.pdf"
              className="bg-transparent border-2 border-white text-white px-8 py-4 rounded-lg font-bold text-lg hover:bg-white/10 transition-colors flex items-center justify-center"
            >
            ...
            <Link
              to="/enroll"
              className="bg-white text-indigo-700 px-8 py-4 rounded-lg font-bold text-lg hover:bg-gray-300 transition-colors shadow-lg hover:shadow-xl"
            >
              Enroll Now
            </Link>
          </div>
